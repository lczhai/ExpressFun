...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
		...
		//
		itemWidth = ([UIScreen mainScreen].bounds.size.width - (numbersOfItemInLine + 1)*itemHorPadding)/numbersOfItemInLine;
...
//                                                                       bgViewHeight)];
    ...
    UIView *shareActionView = [[UIView alloc]initWithFrame:CGRectMake(0,[UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width,0)];
    shareActionView.tag = 1001;
    [UIView animateWithDuration:0.2 animations:^{
        
        shareActionView.frame = CGRectMake(0,[UIScreen mainScreen].bounds.size.height-120,[UIScreen mainScreen].bounds.size.width,120);
    }];
    
    
    
    ViewRadius(shareActionView, 2);
    shareActionView.backgroundColor = [UIColor colorWithWhite:0.843 alpha:1.000];//设置分享视图颜色
...
- (void)clickClose
{
    UIView *shareActionView = [self viewWithTag:1001];
    
    
    
        [UIView animateWithDuration:0.3 animations:^{
       shareActionView.frame = CGRectMake(0,[UIScreen mainScreen].bounds.size.height,[UIScreen mainScreen].bounds.size.width,0);
